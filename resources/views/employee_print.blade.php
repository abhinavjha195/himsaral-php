...
@if(count($search_result)>0) 
	...
	<div class="print-area"> 			
		...
		<hr color="black">  
		@if ( $i == 0 )  
        <p style="text-align:center;font-size:15px;"><b>School Employee List</b></p>@endif  			
		<table class="" style="margin-left:auto;margin-right:auto;width:580px;border:1px solid;">		 			
			<tbody> 
				...
				@if(count($search_result)>0)	
					...
					<tr class="stutble">		
						<td>{{ $search_result[$k]->emp_no }}</td>	
						<td>{{ $search_result[$k]->emp_name }}</td>
						<td>{{ $search_result[$k]->father_name }}</td>	
						<td>{{ $search_result[$k]->mother_name }}</td>	
						<td>{{ $search_result[$k]->department_name }}</td>	  			
						<td>{{ $search_result[$k]->designation }}</td>
						<td>{{ $search_result[$k]->mobile }}</td>	
						<td>{{ $search_result[$k]->email }}</td>      		
					</tr>
						@endif   		
					@endfor
				@endif 				
