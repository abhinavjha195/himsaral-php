...
<html>
	<head>
		<meta charset="utf-8" />
		<title>Character Certificate Format 2</title>		
	...
	<body>
		<div class="invoice-box">
			<table cellpadding="0" cellspacing="0">
				...
				<tr>
					<td colspan="2">
						<table class="slip" cellpadding="0" cellspacing="10"> 			
							<tr>
							  <td>
							   <p>This is to be certified that {{ucwords($cc[0]->student_name)}} Son/daughter of {{ucwords($cc[0]->father_name)}} and {{ucwords($cc[0]->mother_name)}} has studied from {{date('d-m-Y',strtotime($cc[0]->admission_date))}} To {{date('d-m-Y',strtotime($cc[0]->last_date))}} in
								{{ucwords($cc[0]->className)}} Class({{ucwords($cc[0]->courseName)}}).</p> 
							   <p>He / She has passed/ appeared / failed in {{ucwords($cc[0]->classLast)}} class annual standard examination conducted by {{$cc[0]->exam_board}} in {{ucwords($cc[0]->em_month)}}/{{$cc[0]->exam_year}}  
								under Roll No {{$cc[0]->roll_no}} and has secured {{floatval($cc[0]->secured_marks)}} out of {{floatval($cc[0]->total_marks)}}.</p>			
							  </td>  							  
							</tr>
							<tr>
							  <td>His/Her Date of Birth as per Admission withdrawl Register is:  {{date('d-m-Y',strtotime($cc[0]->dob))}}  
				...
				<tr>		
					<td colspan="2">
						<table cellpadding="2" cellspacing="2" style="margin-top:25px;">   
							<tr>
								<td style="width:45%;"> 		
									Sign. of Incharge
								</td> 								
