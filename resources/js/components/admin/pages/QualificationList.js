...
class QualificationList extends Component {
 ...
 handleDelete(id) {
    Swal.fire({
        ...
        if (result.value) {		//
            axios.delete(`${base_url}api/qualification/delete/${id}`)
                .then((response) => {
...
//return null;
		...
		const url = base_url+'api/qualification/index';
        const columns = ['qualificationName', 'action'];

        let _this = this;

        const options = {
            perPage: 10,
            headings: { qualificationName: 'Qualification Name',  action: 'Actions' },
            sortable: ['qualificationName'],
                ...
                <div className="row page-titles mx-0">
                    ...
                    <div className="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                        <ol className="breadcrumb breadcrumb-btn">
                            <li>
							 <a href={`/qualification_add`} className="btn bg-blue-soft text-blue">
							 <i className="fa fa-user-plus"> Add Qualification</i>
							 </a>
							 </li>
                        </ol>
                    </div>
                                                        ...
                                                        function (row, column) {
                                                            switch (column) {
                                                                case 'action':
                                                                    return (
                                                                        <span>
                                                                            <a href={`/qualification_edit/${row.qualificationId}`} className='btn' data-toggle="tooltip" title="Edit">
                                                                                <i className="fa fa-edit" aria-hidden="true"></i></a>
                                                                            <button className="btn" onClick={() => _this.handleDelete(`${row.qualificationId}`)}>
