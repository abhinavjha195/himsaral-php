  ...
  const data = {
    ...
    this.setState({ isSpinner: true }, () => {
        axios.post(`${base_url}api/teacherclass/update/${id}`,data).then(response => {
            ...
            // {
            //     this.setState({ showError:false,showSuccess:true,message:response.data.message,errors:response.data.errors, isSpinner: false});
            //     window.location.href = base_url+"assign_class_teacher_list";
            // }
            // else
            // {
            //     this.setState({ showError: true, showSuccess:false,message:response.data.message,errors:response.data.errors, isSpinner: false});
            // }
        })
        .catch(error => {
        console.log(error.message);
...
// 		axios.get(`${base_url}api`+`/examdatesheet/section_class_list_by_course/${exam_id}/${list}`).then(response => {
//             console.log("section_class_list_by_course",response.data.data);
// 			// this.setState({
// 			// 	coursearr:unique_arr,
// 			// 	sectionData: response.data.data?response.data.data:[],
// 			// });
...
handleCourse(event) {
    ...
    const check_val = parseInt(event.target.value);
    const exam_id = this.state.exam_id;

    let updatedCourseArr = [];

    if (check) {
        updatedCourseArr.push(check_val);
    } else {
        updatedCourseArr = this.state.coursearr.filter(id => id !== check_val);
    }

    this.setState({ coursearr: updatedCourseArr }, () => {
        if (updatedCourseArr.length > 0) {
            const courseId = updatedCourseArr[0]; // Send only the first (and only) selected course ID
            axios.get(`${base_url}api/examdatesheet/section_class_list_by_course/${exam_id}/${courseId}`)
                .then(response => {
                    this.setState({
                        sectionData: response.data.data ? response.data.data : [],
                    });
						...
						<div className="basic-form form-own email-form">
                                        ...
                                        <table className="table table-bordered table-striped verticle-middle table-responsive-sm">
                                            ...
                                            <tbody>
                                            {
                                                this.state.sectionData.map((item,key) => {
                                                    const section_arr1 = item.sectionId;
                                                    const section_arr2 = item.sectionName;
                                                    const arc=item.courseId+item.classId;
                                                    ...
                                                    return (
                                                        <tr key={key}>
                                                        <td>
                                                        <div className="form-checkbox"><input type="checkbox" className="form-check-input" name="course[]" checked={(this.state.corearr.hasOwnProperty(courseId) && (this.state.corearr[courseId].match(new RegExp("(?:^|,)"+classId+"(?:,|$)"))))?true:false} id={courseId} value={classId} onChange={this.handleCourseRow} ref={node =>this.state.classarr.push(node)}/>
                                                        </div>
                                                        </td>
                                                        <td>{courseName}</td>
                                                        <td>{className}</td>
                                                        ...
                                                        <div className="form-check form-checkbox">
                                                                <input
                                                                    ...
                                                                    checked={(this.state.sectionarr.hasOwnProperty(classId) && (this.state.sectionarr[classId].includes(section_arr1)))}
                                                                    id={classId}
                                                                    name="section[]"
                                                                    value={section_arr1}
                                                                    onChange={this.handleSection}
                                                                    ref={node => this.state.sectionlst.push(node)}
                                                                />
						  ...
						  <div className="col-sm-12">
							<div className="submit-btn form-own">
                                <button type="submit" className="btn btn-primary btn-sm mx-1" disabled={this.state.isSpinner}> View <span className="kt-spinner kt-spinner--sm kt-spinner--right kt-spinner--light" style={{ display: this.state.isSpinner ? 'inline' : 'none' }}></span></button>
							</div>
						  </div>
