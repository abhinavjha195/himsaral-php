...
class SectionList extends Component {
    ...
    handleDelete(id) {
        ...
        Swal.fire({
            ...
            if (result.value) {		//
                axios.delete(base_url+`api/delete_section_process/${id}`)
                    .then((response) => {
...
//return null;
        ...
        const url = base_url+'api/section_list';
        const columns = ['courseName', 'className', 'sectionName', 'status', 'action'];

        let _this = this;

        const options = {
            perPage: 10,
            headings: { courseName: 'Course Name', className: 'Class Name', sectionName: 'Section Name', Remark: 'Remarks', action: 'Actions' },
            sortable: ['courseName', 'className', 'sectionName', 'status', 'Remark',],
                                                ...
                                                <ServerTable ref={this.serverTable} columns={columns} url={url} options={options} bordered condensed striped>
                                                    {
                                                        function (row, column) {
                                                            switch (column) {
                                                                case 'action':
                                                                    return (
                                                                        <span><a href={`/section_edit/${row.sectionId}`} className='btn' data-toggle="tooltip" title="Edit"><i className="fa fa-edit" aria-hidden="true"></i></a>
                                                                            <button className="btn" onClick={() => _this.handleDelete(`${row.sectionId}`)}>
