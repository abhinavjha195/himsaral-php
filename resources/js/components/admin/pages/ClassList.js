...
class ClassList extends Component {
    ...
    handleDelete(id) {
        ...
        Swal.fire({
            ...
            if (result.value) {		//
                axios.delete(base_url+`api/class/delete/${id}`)
                    .then((response) => {
...
//return null;
        ...
        const url = base_url + 'api/class/index';
        const columns = ['courseName', 'className', 'status', 'Remark', 'action'];

        let _this = this;

        const options = {
            perPage: 10,
            headings: { courseName: 'Course Name', className: 'Class Name', Remark: 'Remarks', action: 'Actions' },
            sortable: ['courseName', 'className', 'status', 'Remark',],
                                                ...
                                                <ServerTable ref={this.serverTable} columns={columns} url={url} options={options} bordered striped>
                                                    {
                                                        function (row, column) {
                                                            switch (column) {
                                                                case 'action':
                                                                    return (
                                                                        <span><a href={`/class_edit/${row.classId}`} className='btn' data-toggle="tooltip" title="Edit"><i className="fa fa-edit" aria-hidden="true"></i></a>
                                                                            <button className="btn" onClick={() => _this.handleDelete(`${row.classId}`)}>
