                ...
                <div className="row page-titles mx-0">
                    ...
                    {/*****<div className="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                        <ol className="breadcrumb breadcrumb-btn">
                            <li><a href="./className-wise-subjects-create.html" className="btn bg-blue-soft text-blue"><i className="fa fa-user-plus"></i> Add className Wise Subjects</a></li>
                        </ol>
                    </div>****/}
                </div>
               {/***** row ****/}
                ...
                <div className="row">
                    <div className="col-12">
                              ...
                              <div className="fee-collection">
								...
								<div className="basic-form form-own">
									<div className="form-row">
									  <div className="col-md-6 left">
									    <div className="form-row">
										  <div className="form-group col-sm-6">
											...
											<input type="text" className={`form-control ${this.hasErrorFor('admission_no') ? 'is-invalid' : ''}`} name="admission_no" value={(this.state.admission_no)?this.state.admission_no:''} onChange={this.handleAdmission} ref={this.input}/>
											{this.renderErrorFor('admission_no')}
											<div style={style1}>
											{
											  this.state.suggestions.map((item, index) => (
												<div id={item.admission_no} data-s={item.student_name} data-c={item.className} data-f={item.father_name} data-m={item.mother_name} data-p={item.stationName} data-i={item.student_image} data-mo={item.mobile} data-se={item.section_id} data-cl={item.class_id} data-co={item.course_id} key={item.id} style={style2} onClick={this.setAdmission}>{item.admission_no}-{item.student_name}-{item.className}-{item.father_name}</div>
											  ))
											}
										  </div>
										  ...
										  <div className="form-group col-sm-6">
                                            <button type="button" className="btn btn-primary load-fee" onClick={this.loadFee} disabled={this.state.isSpinner}>Load Fee Detail <span className="kt-spinner kt-spinner--sm kt-spinner--right kt-spinner--light" style={{ display: this.state.isSpinner ? 'inline' : 'none' }}></span></button>
										  </div>

										  <div className="form-group col-sm-6">
										  {
											(showCount>0)?(
										    <input type="button" value="Load Previous Fee Detail" className="btn btn-primary" name="Load Previous Fee" onClick={this.previousFee}/>
											):''
										  }
										  </div>
										...
										<div className="form-group col-sm-6">
										  <div className="fee-receipt-checkbox">
										    <div className="form-check settings-form-radio mrb-15">
										      <input type="radio" className="form-check-input" name="receipt_type" value="auto" checked={this.state.selectedReceipt === "auto"}
											  onChange={this.handleReceipt} ref={this.input}/>
											  <label className="form-check-label" htmlFor="check1">Auto</label>
										      <input type="radio" className="form-check-input" name="receipt_type" value="manual" checked={this.state.selectedReceipt === "manual"}
											  onChange={this.handleReceipt} ref={this.input}/>
											  <label className="form-check-label" htmlFor="check1">Manual</label>
										    </div>{/**** form-check ****/}
											{this.renderErrorFor('receipt_type')}
										  </div>{/**** fee-receipt-checkbox ****/}
										</div>
										{
										(this.state.selectedReceipt=='manual')?(
										<div className="form-group col-sm-6">
										  <select className={`form-control ${this.hasErrorFor('receipt_no')?'is-invalid':''}`} name="manual_receipt" value={(this.state.manual_receipt)?this.state.manual_receipt:''} ref={this.input} onChange={this.handleChange}>
											  <option key={0} value="">Select Fee Receipt</option>
											  {receiptArr.map((item,key) => (
											  <option key={item.sno} value={item.sno}>{item.sno}, {item.Admission_No}, {(item.FeeMonth).replace(/[0-9]/g, '')}, {parseFloat(item.FeeAmount).toFixed(2)}</option>
											  ))}
										  </select>
										  {this.renderErrorFor('receipt_no')}
										</div>
										...
										<div className="form-group col-sm-6">
										  {
										(showCount>0)?(<label>Previous Session Pending Fee:</label>
											):''}
										</div>
										<div className="form-group col-sm-6">
										{
										(showCount>0)?(
										  <input className="form-control text-box single-line valid" name="fee_pending" type="text" value={(this.state.fee_pending)?this.state.fee_pending:'0.0'} onChange={this.handleChange} ref={this.input} readOnly/>
											):''
										}
										</div>
									  </div>{/**** form-row ****/}
									  ...
									  <div className="col-md-6 right mrb-30">
									     <div className="single-student-detail">
										   <h4 style={style}>{(this.state.student_name)?this.state.student_name:'Student name'}</h4>
										   <figure><img className="user-profile" src={(this.state.student_image=='')?base_url+'images/student.jpg':base_url+'uploads/student_image/'+this.state.student_image}/></figure>
										   <div className="table-responsive">
										     <table className="table table-bordered">
											   <tbody>
												   <tr>
													 <td>Class Name</td>
													 <td style={style}>{(this.state.class_name)?this.state.class_name:"Student's Class Name"}</td>
												   </tr>
												   <tr>
													 <td>Father Name</td>
													 <td style={style}>{(this.state.father_name)?this.state.father_name:"Student's Father Name"}</td>
												   </tr>
												   <tr>
													 <td>Mother Name</td>
													 <td style={style}>{(this.state.mother_name)?this.state.mother_name:"Student's Mother Name"}</td>
												   </tr>
												   <tr>
													 <td>Mobile No</td>
													 <td>{(this.state.mobile_no)?this.state.mobile_no:"Student's Mobile No."}</td>
												   </tr>
												   <tr>
													 <td>Bus Pick-up Point</td>
													 <td style={style}>{(this.state.pick_up)?this.state.pick_up:"Student's Pick-up Point"}</td>
												   </tr>
											   </tbody>
										     </table>
								...
								<div className="form-own pending-fee-table fee-chart-table">
								  <div className="table-responsive">
									<table className="table table-bordered table-striped verticle-middle table-responsive-sm">
										<thead>
											<tr>
												<th scope="col"><div className="form-checkbox"><input type="checkbox" className="form-check-input" checked={(this.state.catarr.includes(parseInt(0)))?true:false} value="0" onChange={this.checkAll} ref={node =>this.state.catsarr.push(node)}/></div><span>Fee Category</span></th>
												{monthList}
												<th scope="col"></th>
											</tr>
										</thead>
										<tbody>
											{
												...
												if(item.changeable)
												{
													var last_row = (<a href={'#'} className='btn' onClick={(e) => this.handleRow(e,item.fee_id)}>
													<i className="fa fa-edit" aria-hidden="true"></i>
													</a>);
												}
												else
												{
													var last_row = (<span></span>);
												}

											return (
											<tr key={i}>
												<td key={i}><div className="form-checkbox"><input type="checkbox" className="form-check-input" checked={(this.state.catarr.includes(parseInt(item.fee_id)))?true:false} value={item.fee_id} onChange={this.handleCategory} ref={node =>this.state.catsarr.push(node)}/><span>{item.name}</span></div></td>
												{
													Object.keys(monthRows).map((key,index) => {

													const fee_arr=[];

												 if (feeArr.hasOwnProperty(item.fee_id))
												 {
													 if(feeArr[item.fee_id].hasOwnProperty(monthRows[key]))
													 {
														/* var amnt2 = (paidArr[item.fee_id][monthRows[key]]==null)?0:parseFloat(paidArr[item.fee_id][monthRows[key]]);

														var amnt3 = (currentArr[item.fee_id][monthRows[key]]==null)?0:parseFloat(currentArr[item.fee_id][monthRows[key]]);  */

														var amnt = parseFloat(feeArr[item.fee_id][monthRows[key]]);
														var clr = colorArr[item.fee_id][monthRows[key]];
														...
														if(this.state.calcarr.includes(item.fee_id))
														{
															return (
																<td key={index} className={cls}>
																	<input type="text" name={'month_'+item.fee_id+'_'+index} id={item.fee_id} color-id={cls} month-id={monthRows[key]} className="form-control" value={(this.state.columnfee.hasOwnProperty('month_'+item.fee_id+'_'+index))?this.state.columnfee['month_'+item.fee_id+'_'+index]:parseFloat(amnt).toFixed(2)} onChange={this.handleFee}/>
																	{
																	this.state.alertarr.includes('month_'+item.fee_id+'_'+index)?(
																		<span style={{color:"red",fontSize:"15px"}}>Unable to change the fee now as it has been already paid</span>
																		):''
																    }
																</td>
															);
														...
														else
														{
															return (
																<td key={index} className={cls}>
																	{(this.state.columnfee.hasOwnProperty('month_'+item.fee_id+'_'+index))?this.state.columnfee['month_'+item.fee_id+'_'+index]:parseFloat(amnt).toFixed(2)}
																</td>
															);
														}
													 ...
													 else
													 {
														...
														if(this.state.calcarr.includes(item.fee_id))
														{
															return (
																<td key={index} className={cls}>
																<input type="text" id={item.fee_id} color-id={cls} month-id={monthRows[key]} name={'month_'+item.fee_id+'_'+index} className="form-control" value={(this.state.columnfee.hasOwnProperty('month_'+item.fee_id+'_'+index))?this.state.columnfee['month_'+item.fee_id+'_'+index]:0.0} onChange={this.handleFee} />
																{
																	this.state.alertarr.includes('month_'+item.fee_id+'_'+index)?(
																	<span style={{color: "red"}}>Unable to change the fee now as it has been already paid</span>
																	):''
																 }
																</td>
															);
														...
														else
														{
															return (
																<td key={index} className={cls}>
																	{(this.state.columnfee.hasOwnProperty('month_'+item.fee_id+'_'+index))?this.state.columnfee['month_'+item.fee_id+'_'+index]:0.0}
																</td>
															);
														}
								...
								{
								(showCount>0)?(
								<div className="amount-code form-own">
								  <div className="row">

									<div className="col-sm-6 col-lg-3 form-group">
									  <label>Total Amount</label>
									  <input className="form-control text-box single-line valid" name="total_amount"
									  type="text" value={(this.state.total_amount)?this.state.total_amount:'0.0'} onChange={this.handleChange} ref={this.input} readOnly/>
									  {this.renderErrorFor('total_amount')}
									</div>

									<div className="col-sm-6 col-lg-3 form-group">
									  <label>Current Payment</label>
									  <input className={`form-control text-box single-line ${this.hasErrorFor('pay_amount') ? 'is-invalid' : ''}`}  name="paid_amount" type="text" value={(this.state.paid_amount)?this.state.paid_amount:'0.0'} onChange={this.handleBalance} ref={this.input}/>
									  {this.renderErrorFor('pay_amount')}
									</div>

									<div className="col-sm-6 col-lg-3 form-group">
									  <label>Balance</label>
									  <input className="form-control text-box single-line" name="balance" type="text" value={(this.state.balance)?this.state.balance:'0.0'} onChange={this.handleChange} ref={this.input} readOnly/>
									  {this.renderErrorFor('balance')}
									</div>

									<div className="col-sm-6 col-lg-3 form-group">
									  <label>Payment Mode</label>
									  <select className={`form-control ${this.hasErrorFor('pay_mode') ? 'is-invalid' : ''}`} name="payment_mode" value={(this.state.payment_mode)?this.state.payment_mode:pay_opt} onChange={this.handlePayment} ref={this.input}>
										  <option value="">--Select--</option>
										  {modeList}
									  </select>
									  {this.renderErrorFor('pay_mode')}
									</div>

									<div className="col-md-12 PayMode">
									{
									 (this.state.pay_mode=='Cash' || this.state.pay_mode=='Cheque' || this.state.pay_mode=='Swipe')?(
									  <div className="form-row">
									    <div className="form-group col-sm-6 col-lg-3">
										  <label>Bank Name :</label>
										  <input className={`form-control text-box single-line ${this.hasErrorFor('bank_name') ? 'is-invalid' : ''}`} name="bank_name" type="text"  value={(this.state.bank_name)?this.state.bank_name:''} onChange={this.handleChange} ref={this.input}/>
										  {this.renderErrorFor('bank_name')}
									    </div>

										<div className="form-group col-sm-6 col-lg-3">
										  <label>Branch Address :</label>
										  <input className="form-control text-box single-line" name="branch_address" type="text" value={(this.state.branch_address)?this.state.branch_address:''} onChange={this.handleChange} ref={this.input}/>
										  {this.renderErrorFor('branch_address')}
										</div>

										<div className="form-group col-sm-6 col-lg-3">
										  <label>Draft / Cheque No. :</label>
										  <input className="form-control text-box single-line" name="draft_no" type="text"  value={(this.state.draft_no)?this.state.draft_no:''} onChange={this.handleChange} ref={this.input}/>
										  {this.renderErrorFor('draft_no')}
									    </div>

										<div className="form-group col-sm-6 col-lg-3">
										<label>Attachment :</label>
										  <div className="custom-file">
											<input type="file" className="custom-file-input" name="attachment" onChange={this.handleAttachment}/>
											<label className="custom-file-label" htmlFor="customFile">{(this.state.file_lebel)?this.state.file_lebel:'Choose file'}</label>
											{this.renderErrorFor('attachment')}
										  </div>
								    ...
								    <div className="col-sm-6">
								      <h4 className="card-title-btm"><span className="btn bg-red-soft text-red"></span> <small className="text-red">Zero Paid</small> <span className="btn bg-green-soft text-green"></span> <small className="text-green">Full Paid</small> <span className="btn bg-purple-soft text-purple"></span> <small className="text-purple">Partial Paid</small> <span className="btn bg-blue-soft text-blue"></span> <small className="text-blue">NA</small></h4>
								    </div>

								    <div className="col-sm-6">
									  <div className="submit-btn form-own text-right">
										<button type="submit" name="saveonly" className="btn btn-primary btn-sm mx-1" disabled={this.state.isSpinner2}> Save <span className="kt-spinner kt-spinner--sm kt-spinner--right kt-spinner--light" style={{ display: this.state.isSpinner2 ? 'inline' : 'none' }}></span></button>
										<button type="submit" name="printsave" className="btn btn-primary btn-sm mx-1" disabled={this.state.isSpinner3}> Save & Print <span className="kt-spinner kt-spinner--sm kt-spinner--right kt-spinner--light" style={{ display: this.state.isSpinner3 ? 'inline' : 'none' }}></span></button>
									  </div>
								    </div>
								  </div>{/********* row *****/}
								</div>
						...
						<Modal show={this.state.showModal} className="feeModal" onHide={this.handleClose}>
							<Modal.Header closeButton>
							  <Modal.Title>Previous Fee Details</Modal.Title>
							...
							<div className="table-responsive" style={{maxHeight:"400px"}}>
							  <table className="table table-bordered table-striped table-hove">
								<thead>
								  <tr>
									...
									<div className="checkbox">
										<label><input type="checkbox" name="receipt" checked={(this.state.delarr.includes(parseInt(0)))?true:false} value="0" onChange={this.chkAll} ref={node =>this.state.delarr.push(node)}/></label>
									</div>
									</th>
									<th>Receipt No.</th>
									<th>Fee Date</th>
								...
								<tbody>
								 {previousArr.map((item,key) => (
								  <tr key={key}>
									<td><input type="checkbox" name="receipt" checked={(this.state.delarr.includes(parseInt(item.FeeTransId)))?true:false} value={item.FeeTransId} onChange={this.handleTransact} ref={node =>this.state.delarr.push(node)}/></td>
									<td>{item.ReceiptNo}</td>
									<td>{item.FeeDate}</td>
									<td>{item.FeeMonth.replace(/[^A-Za-z]/g,'')}</td>
									<td>{parseFloat(item.fee_sum).toFixed(2)}</td>
									<td>{parseFloat((item.fee_balance==null)?0.0:item.fee_balance).toFixed(2)}</td>
									<td>{(parseFloat(item.fee_sum)+parseFloat((item.fee_balance==null)?0.0:item.fee_balance)).toFixed(2)}</td>
									<td>
									<button id='normal' className="btn" onClick={(e) => this.handlePrint(e,`${item.FeeTransId}`)}>
									<i className="fa fa-print" title='Normal Print' aria-hidden="true"></i>
									</button>
									<button id='thermal' className="btn" onClick={(e) => this.handlePrint(e,`${item.FeeTransId}`)}>
									<i className="fa fa-print" title='Thermal Print' aria-hidden="true"></i>
									</button>
									<button className="btn" onClick={() => this.handleDelete(`${item.FeeTransId}`)}>
							...
							(previousArr.length >0)?(
							<Modal.Footer>
							  <Button variant="info" onClick={this.handleDetail}>
							   Print
							  </Button>
							  <Button variant="danger" onClick={this.deleteSelected}>
							   Delete Selected
							  </Button>
						...
						<Modal show={this.state.showTrasactModal}>
							<Modal.Header>Previous Fee Details</Modal.Header>
							  <Modal.Body>
							  {
									...
									<tbody>
									 {transactionArr.map((item,key) => (
									  <tr key={key}>
										...
										<td>{item.FeeMonth.replace(/[^A-Za-z]/g,'')}</td>
										<td>{item.cat_name}</td>
										<td>{(parseFloat(item.paid_fee)+parseFloat((item.pending_fee==null)?0.0:item.pending_fee)).toFixed(2)}</td>
										<td>{parseFloat(item.paid_fee).toFixed(2)}</td>
										<td>{parseFloat((item.pending_fee==null)?0.0:item.pending_fee).toFixed(2)}
										</td>
									  </tr>
									  ))}
									</tbody>
								...
								{this.state.showErr ?
								 <div className="alert alert-danger" style={{color:"brown"}}>
									<strong>{this.state.invmessage}</strong>
								  </div>
								 : null}
							  </Modal.Body>
							  <Modal.Footer>
								  <Button variant="info" onClick={this.handleInvoice}>
								   Print
								  </Button>
								  <Button variant="light" onClick={this.handleButton}>
								   Close
								  </Button>
							  </Modal.Footer>
						</Modal>
