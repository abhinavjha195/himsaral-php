...
class PaymentModeList extends Component {
    ...
    handleDelete(id) {
        Swal.fire({
            ...
            if (result.value) {		//
                axios.delete(base_url+`api/paymentmode/delete/${id}`)
                    .then((response) => {
...
//return null;
        ...
        const url = base_url + 'api/paymentmode/index';
        const columns = ['pay_mode', 'pay_type', 'action'];

        let _this = this;

        const options = {
            perPage: 10,
            headings: { pay_mode: 'Payment Mode', pay_type: 'Payment Type', action: 'Actions' },
            sortable: ['pay_mode', 'pay_type'],
                            ...
                            <div className="row page-titles mx-0">
                                ...
                                <div className="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                                    <ol className="breadcrumb breadcrumb-btn">
                                        <li>
                                            <a href={`/payment_mode_add`} className="btn bg-blue-soft text-blue">
                                                <i className="fa fa-user-plus">Add Payment Mode</i>
                                            </a>
                                        </li>
                                                ...
                                                <ServerTable ref={this.serverTable} columns={columns} url={url} options={options} bordered striped>
                                                    {
                                                        function (row, column) {
                                                            switch (column) {
                                                                case 'action':
                                                                    return (
                                                                        <span><a href={`/payment_mode_edit/${row.id}`} className='btn' data-toggle="tooltip" title="Edit"><i className="fa fa-edit" aria-hidden="true"></i></a>
