...
class AssignTeacherClass extends Component {
...
handleCreate (event) {
  ...
  axios.post(`${base_url}`+'api/teacherclass/create',data).then(response => {
...
componentDidMount() {
    ...
    axios.get(`${base_url}api`+'/teacherclass/employeeIds').then(response => {
        // console.log("emp resp is",response);
		this.setState({
            employeeIds: response.data.data ? response.data.data : [],
			});
		})
	.catch(error => {
	   console.log(error.message);
		...
		<div className="row page-titles mx-0">
			<div className="col-sm-6 p-md-0">
				<div className="welcome-text">
					<h4>Assign New Class to Teacher</h4>
				</div>
			</div>
							...
							<div className="form-row">
							  ...
							  <div className="form-group col-sm-6">
								<label>Choose Employee ID</label>
                                    <select className={`form-control ${this.hasErrorFor('employee_no') ? 'is-invalid' : ''}`} name="emp_no" value={this.state.employee_no || ''}>
										<option value="">--Choose--</option>
                                        {this.state.employeeIds.map((item, key) => (
                                            <option key={key} value={item.id}>{item.emp_no}</option>
                                        ))}
								    </select>
								{/* <input type="text" className={`form-control input-daterange-timepicker ${this.hasErrorFor('employee_no') ? 'is-invalid' : ''}`} name="emp_no" placeholder=""/> */}
								{this.renderErrorFor('employee_no')}
							  </div>

							  <div className="form-group col-sm-6">
								<label>Date of Joining</label>
								<input type="date" className={`form-control input-daterange-timepicker ${this.hasErrorFor('date_join') ? 'is-invalid' : ''}`} name="doj" value={(this.state.doj)?this.state.doj:''} onChange={this.handleChange} placeholder="dd/mm/yy"/>
