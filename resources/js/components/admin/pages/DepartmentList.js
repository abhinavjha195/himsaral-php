...
class DepartmentList extends Component {
    ...
    handleDelete(id) {
        Swal.fire({
            ...
            if (result.value) {		//
                axios.delete(base_url + `api/department/delete/${id}`)
                    .then((response) => {
...
//return null;
        ...
        const url = base_url + 'api/department/index';
        const columns = ['departmentName', 'action'];

        let _this = this;

        const options = {
            perPage: 10,
            headings: { departmentName: 'Department Name', action: 'Actions' },
                            ...
                            <div className="row page-titles mx-0">
                                ...
                                <div className="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                                    <ol className="breadcrumb breadcrumb-btn">
                                        <li>
                                            <a href={`/department_add`} className="btn bg-blue-soft text-blue">
                                                <i className="fa fa-user-plus"> Add Department</i>
                                            </a>
                                        </li>
                                    </ol>
                                </div>
                                                        ...
                                                        function (row, column) {
                                                            switch (column) {
                                                                case 'action':
                                                                    return (
                                                                        <span>
                                                                            <a href={`/department_edit/${row.departmentId}`} className='btn' data-toggle="tooltip" title="Edit">
                                                                                <i className="fa fa-edit" aria-hidden="true"></i></a>
                                                                            <button className="btn" onClick={() => _this.handleDelete(`${row.departmentId}`)}>
