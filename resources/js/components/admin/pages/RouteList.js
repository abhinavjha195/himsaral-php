...
class RouteList extends Component {
    ...
    handleDelete(id) {
        Swal.fire({
            ...
            if (result.value) {		//
                axios.delete(base_url+`api/route/delete/${id}`)
                    .then((response) => {
...
//return null;
        ...
        const url = base_url+'api/route/index';
        const columns = ['routeNo', 'stations', 'action'];		

        let _this = this;

        const options = {
            perPage: 10,
            headings: { routeNo: 'Route No', stations: 'Station List', action: 'Actions' },
            sortable: ['routeNo', 'stations'],
                            ...
                            <div className="row page-titles mx-0">
                                ...
                                <div className="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                                    <ol className="breadcrumb breadcrumb-btn">
                                        <li>
                                            <a href={`/route_add`} className="btn bg-blue-soft text-blue">
                                                        ...
                                                        function (row, column) {
                                                            switch (column) {
                                                                case 'action':
                                                                    return (
                                                                        <span>
                                                                            <a href={`/route_view/${row.routeId}`} className='btn'>
                                                                                <i className="fa fa-eye" aria-hidden="true"></i></a>
                                                                            <a href={`/route_edit/${row.routeId}`} className='btn' data-toggle="tooltip" title="Edit">
                                                                                <i className="fa fa-edit" aria-hidden="true"></i></a>
                                                                            <button className="btn" onClick={() => _this.handleDelete(`${row.routeId}`)}>
